            ...
            var y = function(f, c, e) {
                    ...
                    if ("Yahoo! Weather Error" != c.description) {
                      ...
                      //  e.image && (b += ' style="background-image: url(http://l.yimg.com/a/i/us/nws/weather/gr/' + c.item.condition.code + daynight.substring(0, 1) + '.png); background-repeat: no-repeat;"');http://l.yimg.com/a/i/us/nws/weather/gr
                        e.image && (b += ' style="background-image: url(img/icons/' + c.item.condition.code +daynight.substring(0, 1) +'.png); background-repeat: no-repeat;margin-left: 19px;background-position-x: 353px;"');
                       // b = b + ">" + 
						//	('');
                        //e.country && (b += '<div class="weatherCountry">' + c.location.country + "</div>");
                        b += '<div class="weatherTemp">' + c.item.condition.temp + "&deg; ";
                       ...
                       /* e.highlow &&
                        ...
                        /*if (e.forecast) {
                            b += '</div><div class="weatherForecast">';
                            a = c.item.forecast;
                            for (g = 0; g < a.length; g++) b += '<div class="weatherForecastItem'+g+'" style="background-image: url(img/icons/' + a[g].code + 'd.png); background-repeat: no-repeat;  background-size: contain;background-position: 27px center;">', b += '<div class="weatherForecastDay'+g+'  txt">' + a[g].day + "</div>", b += '<div class="weatherForecastText'+g+'" style="margin-top: 116px;color: #848383;font-size: 20px;  margin-bottom: -23px;height: 46px;overflow: hidden;">' + a[g].text + "</div>", b += '<div class="weatherForecastRange'+g+'" style="margin-top: 50px;width: 148px;  height: 38px;">	<div class="high">' + a[g].high +"&deg;</div><div class='left'>" + a[g].low +"&deg;</div></div>", b += "</div>";
                            b += "</div>"
                        }*/
